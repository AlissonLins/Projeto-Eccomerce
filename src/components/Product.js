    ...
    <div>
      ...
      overflow-hidden group transition'>
        ...
        gap-y-2 opacity-0 group-hover:opacity-100 transition-all duration-300'>
          ...
          <Link to={`/product/${id}`} className='w-12 h-12 bg-white flex justify-center items-center text-primary
          drop-shadow-xl'>
            <BsEyeFill />
          </Link>
        </div>
      ...
      <div>
        ...
        <h2 className='font-semibold'>R$: {parseFloat(price).toFixed(2)}</h2>
      </div>
    </div>
  );
};
